...
for j in range(1,11):
    ...
    # 定义客户端数量
    num_clients = 1  # 假设有N个客户端
    Total_clients=30
    cluster_num = 30 # 假设有M个集群
    batch_size=64
    # 分割数据集为IID
    ...
    for _ in range(cluster_num):
        ...
        # 创建对应的服务器端优化器
        server_optimizers = [optim.Adam(server_model.parameters(), lr=0.001) for server_model in server_models]
    ...
    for round in range(num_rounds):
        ...
        for idx, model_group in enumerate(models_groups):
            ...
            server_models = model_group["server_models"]
            client_optimizers = model_group["client_optimizers"]
            server_optimizers = model_group["server_optimizers"]
            ...
            #训练部分
            for i, (client_model, client_optimizer,server_model, server_optimizer) in enumerate(
                    zip(client_models, client_optimizers,server_models, server_optimizers)):
        ...
        # 所有模型组中的服务器模型
        server_models = [server_model for model_group in models_groups for server_model in model_group["server_models"]]
        # 服务器模型联邦平均
        averaged_server_state_dict = federated_average(server_models)
        # 更新每个服务器模型
        for model_group in models_groups:
            for server_model in model_group["server_models"]:
                server_model.load_state_dict(averaged_server_state_dict)

        # 所有模型组中的客户端模型
        client_models = [client_model for model_group in models_groups for client_model in model_group["client_models"]]

        # 客户端模型联邦平均
        averaged_client_state_dict = federated_average(client_models)
        ...
        # accuracy = test(client_models[5], aggregated_server_model, test_loader)
    ...
    results_path = f'./SFL_noniid0{j}.xlsx' if j < 11 else f'./SFL_noniid{j}.xlsx'
